...
|---|---|---|
| File navigator (Develop), Track canvas nodes, Desk reference nodes | Overflow menu (⋯) on hover — "Add to Desk" is one option | Note action sets will grow; hover-revealing on every item in a dense list would be noisy |
| Quick Look modal, Develop editor | Direct button at the top of the interface | Space exists to surface it directly |
