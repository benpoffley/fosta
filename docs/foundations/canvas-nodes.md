...
Freeform nodes behave differently enough from the other two types that they're documented separately in full — see [Freeform nodes (Desk only)](#freeform-nodes-desk-only) below.
...
**Freeform nodes do not participate in this model** — they are always shown as simple text boxes and cannot be expanded to a live preview, since there's no underlying note to preview. See [Freeform nodes (Desk only)](#freeform-nodes-desk-only).
...
Pinning has a particular significance for freeform nodes specifically — see [Pinning and the third lifecycle option](#pinning-and-the-third-lifecycle-option) below.
...
### Pinning and the third lifecycle option

By default, a freeform node has two possible fates: converted to a real note via "Capture to Inbox," or cleared at the next wipe.
