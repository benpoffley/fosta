---
title: "Roadmap"
parent: "Project"
nav_order: 2
layout: default
---

# Roadmap

**Status:** Directional — lowest authority. Not a commitment.

## v1.0 — The core pipeline

| Phase | Milestone | Key deliverables |
|---|---|---|
| Mo 1 | Foundation | Product brief ✓, wireframes ✓, hi-fi Sort, dev env |
| Mo 2–4 | Capture + Sort | Working inbox, tag bubbles, capture flow, local storage |
| Mo 4–5 | Cloudflare | Work capture, read-only vault snapshot |
| Mo 5–8 | Layer → Share → Track | Editor, transclusion, publishing, Line / Web / Thread canvases |
| Mo 8–9 | v1.0 | Polish, testing, launch |

## v1.1 — Deferred from v1.0

- Split view in Layer
- Track (if not ready post-Sort)
- Live transclusion across Cloudflare

## v2 — Future

- iPad app (SwiftUI)
- Alternative storage backends (Google Drive, iCloud) via StorageAdapter
- Web app (read-only at minimum)
- AI features (auto-backlinks, suggested tags)

## Out of scope entirely

See `scope.md` for the full list.
